...
int copy (string source, string dest, CallBackFunc Report) // callback progressReport
{
    ...
    if (argc < 3) {cout << "Usage: ./upload <target_url> <file1> <file2> ... <fileN>\n";}
        ...
        cout << "exists directory " << argv[1] <<endl;
           ...
           for (int i = 2; i<argc; i++) {
               ...
               auto fileReport = bind(progressReport,i);
               copy(uploadfile, outputPath, fileReport);
           }
